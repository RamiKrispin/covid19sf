...
covid19sf_geo %>% 
  filter(area_type == "ZCTA") %>% 
  mapview(zcol = "count", 
          at = c(0,  200, 400, 800, 1200, 1600, 1800),
...
Plotting `sf` object can be done with the [ggplot2](https://ggplot2.tidyverse.org/) package by using the [geom_sf](https://ggplot2.tidyverse.org/reference/ggsf.html) function for plotting `sf` objects:
